...
class Instructor : public Name{
	protected:
		Name name;
		std::vector<LectureCourse> courses;
	public:
		Instructor(std::string n1, std::string n2) : name(n1, n2){}
...
class Assistant : public Instructor{
	private:
		Name name;
		std::vector<LectureCourse> courses;
	public:
		Assistant(std::string n1, std::string n2): Instructor(n1, n2) {}
		
		void assignLectureCourse(std::vector<LectureCourse> v){
			Instructor::assignLectureCourse(v);
		}
...
class LectureCourse : public Year{
	private:
		std::string title;
		Year year;
...
class Student : public Name, public Year {
	private:
		Name name;
		Year year;
		std::vector<LectureCourse> courses;
